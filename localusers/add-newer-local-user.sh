...
if [[ "${#}" -lt 1 ]]
then
    echo "USAGE: ${0} ACCOUNT [COMMENT]..." >&2
...
# Create a new user
useradd -c "${COMMENT}" -m ${NEW_USERNAME} &> /dev/null

# If, for some reason, the useradd command failed, notify the user and exit
if [[ "${?}" != 0 ]]
then
    echo "Oops, something went wrong and the new user WAS NOT created." >&2
    exit 1
fi

# Set the password for user
echo ${INITIAL_PASSWORD} | passwd --stdin ${NEW_USERNAME} &> /dev/null
...
passwd -e ${NEW_USERNAME} &> /dev/null
